...
} forEach ([_items] call EFUNC(shop,items_difference));
...
private _fnc_onConfirm = {
	...
	private _items = [getUnitLoadout player] call EFUNC(locker,loadout_items);
	[[_items] call EFUNC(shop,items_difference)] call EFUNC(bank,items_purchase);
};

private _cost = [_items, 0] call EFUNC(shop,items_cost);
private _balance = EGVAR(bank,balance);
if (_cost > _balance) then {
	...
	private _personal = [_items, 0] call EFUNC(shop,items_cost);
	private _global = [_items, 1] call EFUNC(shop,items_cost);
